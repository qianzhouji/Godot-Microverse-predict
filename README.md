...
**可调整特质**:
- **p_base**: 离开阈值（对环境平均奖赏的估计）
- **eta_s**: 初始奖赏感知权重
- **eta_a**: 衰减率感知权重
- **beta_effort**: 努力敏感性（核心差异参数）
...
**流程**:
1. **反思分析**: LLM分析当日记忆，识别情绪主题和关键事件
2. **参数调整决策**: LLM判断四项认知参数的调整方向（↑/↓/→）和严重程度（1-5）
3. **动态幅度计算**: 严重程度映射到基础幅度（1%→12%），乘以个体差异乘数
4. **PHQ-9完整评估**: LLM评估九项症状（0-3分），计算总分和等级
5. **更新与记录**: 更新抑郁水平，记录反思记忆
